...
echo "*********************************************************************"
echo "*********************************************************************"
echo "         START OF PROCESSING EXPENSE AND REVENUE FILES"
echo "*********************************************************************"
echo "*********************************************************************"
...
Lib="/Users/ktwitchell001/.ivy2/cache/org.scala-lang/scala-library/jars"
JarPath="/Users/ktwitchell001/workspace/VAPostingDemo/target"
inputPath="/Users/ktwitchell001/workspace/VAPostingDemo/data/NonParallelTestData/"
outputPath="/Users/ktwitchell001/workspace/VAPostingDemo/data/NonParallelTestData/"  #never tested difference betwen input and output paths
...
do
    ...
    echo "File Type: "$filetype" Input File Name: "$inputFile" Output File Name: "$outputTRN
    fiscalCC="20"
    fiscalYY=${file:2:2}
    slash="/"
            ...
            case $fiscalYY in
                03|04|05|06|07|08|09|10|11)
                      ...
                      awk  \
                         ...
                         |
                      sort -k 1 | \
#                      cat $outputPath$outputTRN | \
                      java -cp $JarPath/VAPostingData-0.1-SNAPSHOT.jar:$Lib/*  instIDAssign \
                            $inputPath $outputPath $maxIDFileName $instIDFileName \
                             $outputTRN $instUPDFileName
                      ;;
                12|13)
                ...
                03|04|05|06|07|08|09|10|11)
                    ...
                    # Run 2003 - 2011 REVENUE
                      ...
                      awk  \
                         ...
                         FNR==NR{gsub ("\015","\015\012", $0);(!/^\"/);hash[$2]=$3;next} \
                         !/^\"/ \
                         { \
                         printf "%s,","\""$1"\""; \
                         printf "%s,",$1; \
    ...
    echo "Phase 1 End Seconds: "$SECONDS
    ...
    backup () {
        for file in $inputPath"$@"; do
            ...
            printf "Backing up '%s' to '%s'\n" "$inputPath"`basename "$file"` "$inputPath$new"".txt";
            #\cp -ip "$file" "$new";
            \mv "$inputPath"`basename "$file"` "$inputPath$new"".txt";
        done
    }
    ...
    rename () {
        for file in $inputPath"$@"; do
            ...
            printf "Renaming '%s' to '%s'\n" "$inputPath""new"`basename "$file"` "$inputPath$new";
            #\cp -ip "$file" "$new";
            \mv "$inputPath""new"`basename "$file"` "$inputPath$new";
        done
    }

    echo "Sort UPD Transaction File"
    # Sort InstUPD File prior to rename step below
    sort -k 1 $outputPath"new"$instUPDFileName > $outputPath"new"$outputUPDi #the sort order on this might be wrong!!!!!!!
    echo "End Sort UPD Transaction File"

    # backup input files to last execution, excluding the input transaction file
    backup $maxIDFileName $instIDFileName

    echo "Backup complete"

    # rename output files from last execution to make them input files to this execution
    rename $maxIDFileName $instIDFileName $outputTRN $outputUPDi

    echo "Rename complete"

    echo "Phase Sort Start Seconds: "$SECONDS
    sort -t ',' -k 1,1 -k 5,16 -k 18,18 $outputPath$outputTRN |
    ...
    java -cp $JarPath/VAPostingData-0.1-SNAPSHOT.jar$Lib/* instPosting \
#    scala /Users/ktwitchell001/workspace/cdscVADataDemoSys/src/main/scala/instPosting.scala \
         $inputPath $outputPath $maxIDFileName \
        $instTBLFileName $balTBLFileName $tranTBLFileName $outputUPDi $sumTBLFileName
    echo "Phase Phase 2 End Seconds: "$SECONDS

    fileCount=$[fileCount+1]
    echo "File count processed: "$fileCount

    # backup input files to last execution
    backup $maxIDFileName $balTBLFileName $instTBLFileName

    echo "Backup complete"

    # rename output files from last execution to make them input files to this execution
    rename $maxIDFileName $balTBLFileName $instTBLFileName
    ...
    if [ $filetype == "R" ]
        ...
        echo "******************************************************************************************************"
        echo "Renaming Annual Balance File: "$inputPath$balTBLFileName" to "$inputPath$outputBAL
        \mv $inputPath$balTBLFileName $inputPath$outputBAL
        echo "******************************************************************************************************"
        echo "Create new empty balance file for next year's processing"
        \touch $inputPath$balTBLFileName
    fi
